...
int command2_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __string_encode_array(
        buf, offset + pos, maxlen - pos, &exec_str_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    char* command_name_cstr = const_cast<char*>(this->command_name.c_str());
    tlen = __string_encode_array(
        buf, offset + pos, maxlen - pos, &command_name_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int command2_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(
        buf, offset + pos, maxlen - pos, &__exec_str_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__exec_str_len__ > maxlen - pos) return -1;
    this->exec_str.assign(
        static_cast<const char*>(buf) + offset + pos, __exec_str_len__ - 1);
    pos += __exec_str_len__;

    int32_t __command_name_len__;
    tlen = __int32_t_decode_array(
        buf, offset + pos, maxlen - pos, &__command_name_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__command_name_len__ > maxlen - pos) return -1;
    this->command_name.assign(
        static_cast<const char*>(buf) + offset + pos, __command_name_len__ - 1);
