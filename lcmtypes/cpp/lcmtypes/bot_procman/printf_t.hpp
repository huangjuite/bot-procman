...
int printf_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __string_encode_array(
        buf, offset + pos, maxlen - pos, &deputy_name_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int printf_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(
        buf, offset + pos, maxlen - pos, &__deputy_name_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__deputy_name_len__ > maxlen - pos) return -1;
    this->deputy_name.assign(
        static_cast<const char*>(buf) + offset + pos, __deputy_name_len__ - 1);
